...
def test_level3() -> None:
    ...
    try:
        ...
        # Stato critico: modifica h_state direttamente poi aggiorna
        hc.h_state["safety"] = 0.10
        hc.h_state["energy"] = 0.20
        ...
        # Il controller è in scaffold mode (enabled=False) — funziona ma non genera alerts reali
        record("EM-05", 3, "Viability drop genera alert omeostatici",
               "PARTIAL" if viability_drops else "FAIL",
               f"viability: {viability_high:.2f}→{viability_low:.2f} "
               f"alerts={alerts_low} scaffold={scaffold_mode}",
               "Controller in scaffold mode (enabled=False). "
               "Alert reali attivati da DriveExecutive (M7.0)")
    except Exception as e:

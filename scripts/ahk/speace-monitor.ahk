...
WAL_LOG := SPEACE_DIR "\safe-proactive\WAL.log"
...
RunMonitorNow(*) {
    monitor_script := SPEACE_DIR "\scripts\speace_status_monitor.py"
    if FileExist(monitor_script) {
        ...
        MsgBox("speace_status_monitor.py non trovato in:\n" monitor_script, "SPEACE", "Icon!")
    }
}

RunEvolverNow(*) {
    evolver_script := SPEACE_DIR "\scripts\speace-cortex-evolver.py"
